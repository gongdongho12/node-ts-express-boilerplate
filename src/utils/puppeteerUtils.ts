...
export const puppeteerBodyParser = async (
	requestData: IPuppeteerBody
): Promise<Map<string, string>> => {
	const { func, innerFunc, children = [], ...share } = requestData;
	const { url, selector } = share;
	const browser = await getBrowser();
	const page = await browser
		.newPage()
		.then((page) =>
			page.goto(url, { waitUntil: "networkidle0" }).then(() => page)
		);
	const result: Array<string[]> = [];
	const promiseArr: Array<Promise<string>> = [];

	if (func != undefined) {
		const newFunc: string = await page.evaluate(
			({ func }) => {
		...
		if (typeof newFunc == "string") {
			try {
				const resultUpdater = new Function("rest", newFunc)(share);
	...
	if (innerFunc != undefined) {
		try {
			const resultUpdater = new Function("rest", innerFunc)(share);
	...
	if (selector != undefined) {
		const mainPromise: Promise<string> = page.evaluate(
			({ selector }) => {
	...
	if (children.length > 0) {
		const childPromiseList = children.map(async (child: IPuppeteerSelector) => {
			const { func, innerFunc, ...restTemp } = child;
      ...
      */
			if (func != undefined) {
				...
				if (typeof newFunc == "string") {
					try {
						const resultUpdater = new Function("rest", newFunc)(rest);
						Object.keys(resultUpdater).forEach((key) => {
							// @ts-ignore
        ...
        try {
          ...
          // console.log("resultUpdater", resultUpdater);
			...
			const childPromise: Promise<string> = page.evaluate(
	...
	const assignData = await Promise.all(promiseArr).then(
		(dataList: string[]) => {
